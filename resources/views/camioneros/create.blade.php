...
<body>
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link active" aria-current="page" href="{{route('camionero.create')}}">Crear</a>
                </li>
            </ul>
          </div>
        </div>
    ...
    <div class="d-flex justify-content-center align-items-center flex-column" style="height: calc(100vh - 56px);">
        <h1 class="mb-3">Crear Camionero</h1>
        <form class="w-25" action="{{route('camionero.store')}}" method="POST">
            @csrf
            ...
            <div class="form-floating mb-3">
                <input name="poblacion" type="text" class="form-control" id="poblacion" placeholder="poblacion">
            ...
            <div class="form-floating mb-3">
                <input name="salario" type="number" class="form-control" id="salario" placeholder="salario">
                <label for="salario">Salario</label>
            </div>

            <div class="form-floating">
                <select name="camion" class="form-select mb-3" id="camiones" aria-label="Floating label select example">
                  <option selected>Selecciona una opción</option>
                  @foreach($camiones as $camion)
                    <option value="{{$camion->id}}">{{$camion->matricula}}</option>
                  @endforeach
                </select>
                <label for="camiones">Camión</label>
            </div>
